  ...
  return (
    <article className="relative grid min-h-[104px] grid-cols-[92px_minmax(0,1fr)] gap-2.5 rounded-[1.2rem] border border-black/5 bg-white/85 p-2 shadow-[0_10px_24px_rgba(20,20,20,0.055)] min-[431px]:min-h-[112px] min-[431px]:grid-cols-[100px_minmax(0,1fr)] min-[431px]:gap-3 min-[431px]:rounded-[1.35rem] min-[431px]:p-2.5 max-[370px]:min-h-[96px] max-[370px]:grid-cols-[78px_minmax(0,1fr)]">
      ...
      <div className="flex min-w-0 flex-col py-0.5 pr-9 max-[370px]:pr-8">
        <button
          type="button"
          className="truncate border-0 bg-transparent p-0 text-left text-[16px] font-black leading-tight text-[#141414] max-[370px]:text-[15px]"
          onClick={onOpen}
        >
          {item.name}
        </button>
        {item.category ? (
          <span className="mt-1.5 truncate text-xs font-semibold text-neutral-500 max-[370px]:text-[11px]">
            {item.category}
          </span>
        ) : null}
        <strong className="mt-auto text-[17px] font-black text-[#151515] max-[370px]:text-[16px]">{formatBirr(item.unitPrice)}</strong>
      </div>
      <Button
        type="button"
        size="icon"
